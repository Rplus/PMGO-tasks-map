(function (window, L) {
    ...
    function setTasks(tasks) {
        ...
        const events = Object.keys(eventTasks);

        tasks = tasks.reduce((all, task) => {
            let eventGroup = 'x';
            events.some(event => {
              let matched = (eventTasks[event].indexOf(task) !== -1);
              if (matched) {
                eventGroup = event;
              }
              return matched;
        ...
        var results = '<option value="請選擇任務">請選擇任務</option>' +
            Object.keys(tasks).sort((a, b) => a[0] - b[0]).map(group => {
                var optionsHtml = '';
                optionsHtml = tasks[group].sort().map(task => {
